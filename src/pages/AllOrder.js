            ...
            data.map((item,index)=>{
                return (
                  <div key={item.userId+index}>
                    <p className='font-medium text-lg'>{momemt(item.createdAt).format("LL")}</p>
                    <div className='border border-2 rounded p-4'>
                    <div className='flex flex-col lg:flex-row justify-between bg-slate-200'>
                    <div className='grid gap-1'>
                      {
                        item?.productDetails.map((product,index)=>{
                          return (
                            <div key={product.productId+index} className='flex gap-3 bg-slate-100'>
                              <img src={product.image[0]} alt="order-pic" className='w-28 h-28 bg-slate-200 object-scale-down p-2'/>
                              <div>
                              <div className='font-medium text-lg text-ellipsis line-clamp-1'>{product.name}</div>
                              <div className='flex items-center gap-5 mt-1'>
                                <div className='text-red-600 text-lg'>{displayINRCurrency(product.price)}</div>
